...
import { defaultExtensions } from "../util/defaultExtensions";
import type { UseEditorInstanceOptions } from "../types";
...
export function useEditorInstance(opts?: UseEditorInstanceOptions) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "tiptap-Editor prose max-w-none prose-p:m-0 prose-headings:mt-0 prose-headings:mb-1 focus:outline-hidden",
      },
    },
  });
