    ...
    return [
      Suggestion({
        ...
        items: ({ query, editor }: { query: string, editor: Editor }) => {
          ...
          if (
            this.options.suggestion.toggle?.includes(parentType) 
          ) {
            return [];
          }
          ...
          return commands.filter(
            (command) =>
              ...
              (command.hint ?? "").includes(query.toLowerCase())
          );
        },
        render: () => {
          ...
          const onKeyNav = (
            ...
            if (dir === "down")
              selectedIndex = (selectedIndex + 1) % itemsCache.length;
            if (dir === "up") {
              selectedIndex =
                (selectedIndex - 1 + itemsCache.length) % itemsCache.length;
            }
