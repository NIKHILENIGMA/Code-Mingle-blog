...
const ColorSelection: FC<ColorSelectionProps> = ({ editor }) => {
  ...
  return (
    <Select
      onValueChange={(value) => {
        const selectedColor = COLOR_HEX_CODE.find(
        ...
        const selectedHighlight = HIGHLIGHT_HEX_CODE.find(
          (hl) => hl.hex === value
        );
    ...
    >
      ...
      <SelectContent className="overflow-y-auto max-h-72 bg-background text-secondary-foreground">
        <div className="p-1">
          ...
          {COLOR_HEX_CODE.map(
            (color: { hex: string; color: string }, index: number) => (
              <SelectItem
                key={color.hex + index}
                value={color.hex}
              ...
              >
                <div
                  style={{
                    border: `1.5px solid ${
                      color.hex !== "default" ? `${color.hex}` : "black"
                    }`,
                    color: color.hex,
                    opacity: 0.8,
                    padding: "0.6rem",
        ...
        <div className="p-1">
          ...
          {HIGHLIGHT_HEX_CODE.map(
            (color: { hex: string; highLight: string }, index: number) => (
              <SelectItem value={color.hex} key={color.hex + index}>
                <div
                  style={{
