...
import { mockOpenAIStream } from "../util/mockOpenAIStream";
...
const AIChatbar: FC<ReactNodeViewProps> = ({ editor, deleteNode, node }) => {
  ...
  return (
    <NodeViewWrapper contentEditable={false} className="relative">
      <div className="relative w-full px-1 py-1 rounded-md ai-content">
        {response !== '' ? (
          <div className="w-full h-auto mb-2 rounded-md shadow-none" onClick={handleResponseClick}>
          ...
          <p className="py-1 rounded-r-md rounded-l-md text-md text-secondary-foreground/50 bg-primary/10">
          ...
          {phase === "loading" ? (
            ...
            <div className="flex flex-col w-full space-y-1 shadow-md rounded-md border-primary border-[0.1rem] p-1">
              ...
              <div className="flex items-center w-full p-1 space-x-3">
                <Button variant={"default"} onClick={acceptResponse}>
                  <Check /> Accept
                </Button>
                <Button variant={"secondary"} onClick={rejectResponse}>
                  <X /> Reject
                </Button>
              </div>
            </div>
